...
from utils.common import GaussianSmearing, MLP, batch_hybrid_edge_connection, outer_product,global_edge_connection
...
class BaseX2HAttLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_heads, edge_feat_dim, r_feat_dim,
        ...
        # print(f"network:edge_feat_dim:{edge_feat_dim}")
        # print(f"network:r_feat_dim:{r_feat_dim}")
    ...
    def forward(self, h, r_feat, edge_feat, edge_index, e_w=None):
        ...
        hi, hj = h[dst], h[src]
         #for debug
        # print(f"hi.shape: {hi.shape}, hj.shape: {hj.shape}")
        # print(f"r_feat.shape: {r_feat.shape}")
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    def __init__(self, hidden_dim, n_heads, num_r_gaussian, edge_feat_dim, act_fn='relu', norm=True,
        ...
        # print(f"in_network:num_r_gaussian:{num_r_gaussian}")
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def _connect_edge(self, x, mask_ligand, batch):
        ...
        elif self.cutoff_mode == 'global':   #new
            edge_index = global_edge_connection(x=x, batch=batch,mask_ligand=mask_ligand,k=self.knn)
        elif self.cutoff_mode == '~global':   #new
            edge_index = global_edge_connection(x=x, batch=batch,mask_ligand=~mask_ligand,k=self.knn)
        else:
    ...
    def forward(self, h, x, mask_ligand, batch, return_all=False, fix_x=False,return_edge=False):
        ...
        # print(f'x:{x}')
        # print(f'h:{h}')
        edge_indices = []  # store edge_indices for each block
        ...
        if return_edge:
            outputs['edges'] = edge_indices
        
        # for debug
        # 
        # self.print_molecule_nodes(batch, mask_ligand)

        # 
        # self.plot_batch_graph_separate_molecules(x, edge_indices[0], batch, mask_ligand)

        return outputs

    def print_molecule_nodes(self, batch, mask_ligand):
        unique_batches = torch.unique(batch)
        for mol_idx in unique_batches:
            mol_nodes = torch.where(batch == mol_idx)[0]
            print(f"分子 {mol_idx} 的节点: {mol_nodes.tolist()}")
            print(f"分子 {mol_idx} 的节点数量: {len(mol_nodes)}")
            global_nodes = [node.item() for node in mol_nodes if mask_ligand[node]]
            print(f"分子 {mol_idx} 的全局节点: {global_nodes}")
            print()

    def plot_batch_graph_separate_molecules(self, x, edge_indices, batch, mask_ligand):
        unique_batches = torch.unique(batch)
        molecule_nodes = {mol_idx.item(): torch.where(batch == mol_idx)[0] for mol_idx in unique_batches}
        ...
        for edge in edge_indices.T:
            src, dst = edge
            if mask_ligand[src] or mask_ligand[dst]:
                global_edges.append((src.item(), dst.item()))
            else:
                normal_edges.append((src.item(), dst.item()))
        ...
        pos = {}
        for mol_idx in unique_batches:
            mol_idx_int = mol_idx.item()
            if mol_idx_int not in molecule_nodes:
                continue
            mol_node_list = molecule_nodes[mol_idx_int].tolist()
            subgraph = G.subgraph(mol_node_list)

            subgraph_pos = nx.circular_layout(subgraph)
            angle = np.pi * 2 * mol_idx_int / len(unique_batches)
            x_offset = np.cos(angle) * 3  
            y_offset = np.sin(angle) * 3
            for node in subgraph_pos:
                subgraph_pos[node] += np.array([x_offset, y_offset])
        ...
        nx.draw_networkx_edges(G, pos, edgelist=global_edges, edge_color='gray', width=1, style='dashed')

        nx.draw_networkx_labels(G, pos, font_size=8, font_color='black')  

        plt.title("Batch Molecule Structure with Global Nodes")

        plt.savefig("batch_molecule_with_global_nodes_separate.png", dpi=300, bbox_inches='tight')